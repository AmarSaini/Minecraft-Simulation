...
void SoCloud::createCube(float myX, float& myY, float myZ, float& sideLength) {
	...
	for (int i = 0; i < 6; i++) {
		...
		T.push() = GsPnt2(0.5, 0.5);
		T.push() = GsPnt2(0.5, 0.75);
...
void SoCloud::build ( )
 {
   ...
   createCube(xCord, yCord, zCord, sideLen);
   createCube(xCord, yCord, zCord + sideLen, sideLen);
   createCube(xCord, yCord, zCord + (2 * sideLen), sideLen);
   createCube(xCord, yCord, zCord - sideLen, sideLen);
   createCube(xCord + sideLen, yCord, zCord, sideLen);
   createCube(xCord + sideLen, yCord, zCord + sideLen, sideLen);
   createCube(xCord - sideLen, yCord, zCord, sideLen);
   createCube(xCord - sideLen, yCord, zCord + sideLen, sideLen);
   createCube(xCord - sideLen, yCord, zCord - sideLen, sideLen);
...
void SoCloud::draw(const GsMat& tr, const GsMat& pr, const GsLight& l)
{
	float f[4];
